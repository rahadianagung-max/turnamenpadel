...
const netlifyHandler = async (event) => {
  ...
  try {
    ...
    if (path.startsWith("tournament/event/") && path.endsWith("/rename") && method === "POST") {
      return await tRenameEvent(decodeURIComponent(path.replace("tournament/event/", "").replace("/rename", "")), body || {});
    }
...
// Slug used for the friendly /tv/<slug> and /mobile/<slug> URLs. Mirrors the
// client slugify() in tv.html/mobile.html so a stored slug and a name-derived one
// resolve identically.
...
function buildPlayoffTiers(groups, format, N, crossOn) {
  ...
  if (format === "SPLIT") {
    ...
    if (champCount >= 2) emitTier("MAIN", (s) => s.rank <= N);
  }
...
// "Winner Grup A" / "Runner-up Grup C" instead of real teams.
function playoffProjectionRankLabel(rank, label) {
...
function buildPlayoffProjection(groupsInfo, format, N) {
  ...
  const labelOf = {};
  const groups = groupsInfo.map((g) => ({
    label: g.label,
    standings: Array.from({ length: Math.max(0, g.size) }, (_, i) => {
      const rank = i + 1, id = `${g.label}#${rank}`;
      labelOf[id] = playoffProjectionRankLabel(rank, g.label);
      return { entrantId: id, rank, wins: 0, gd: 0, gf: 0 };
    }),
  }));
